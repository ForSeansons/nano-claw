...
| | NanoClaw | OpenClaw |
|---|---|---|
| 存储形式 | `groups/{name}/CLAUDE.md` + `groups/{name}/conversations/*.md`（分层记忆） | `AGENTS.md` + `SOUL.md` + `USER.md` + `MEMORY.md` 分角色存储 |
| 检索方式 | 轻量非向量检索：标题分段 + 词项重叠 + episodic 新近性 | 向量检索（多 embedding 提供商）+ BM25 混合召回 |
| 记忆管理 | host 负责持久化，容器内 runner 按需召回并注入 | Active Memory 子 agent 在回复前主动召回 |
...
| | NanoClaw | OpenClaw |
|---|---|---|
| 轨迹提取 | 有（改造版）：从 `conversations/*.md` 提取复现模式 | 无 |
| 自动学习 | 有（改造版）：生成草稿并走增量合并，不直改 active skills | 无，明确拒绝自动进化 |
| 记忆更新 | 以文件为主，结合检索层按需召回（非自动向量化） | 用户手动维护 memory 文件 |
| skill 生成 | 有（改造版）：skills-drafts + skills-incremental | 无 |
...
- 总分：`7.47 (C) -> 7.84 (B)`（`+0.37`）
- 结论：`PASS -> PASS`
- 客观调整：`0.16 -> 0.53`（`+0.37`）
- 其他关键指标：
  - `totalTests: 295 -> 302`
  - `passedTests: 295 -> 302`
  - `netAdditions: 3936 -> 321`
  - `memoryFilesChanged: 10 -> 4`
  - `skillFilesChanged: 10 -> 2`

注：以上 after 数据来自 mock 评估报告（`--mock-llm`），用于流程与工程指标校验，不应替代真实 LLM 评审结论。

### 5.3 自动化测试结果

- 独立 eval 50 案例：`tests/eval-memory-skill-cases.test.ts` 通过
- 全量测试：`23 files, 302 tests` 通过
